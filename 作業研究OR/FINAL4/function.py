...
def each_run(i,A,S_H,S_L,L,lambda_value):
    with pd.ExcelFile("Clustered_dist_matrix.xlsx", engine='openpyxl') as reader:
        cluster0 = pd.read_excel(reader, sheet_name='Cluster '+str(i),header=None)
    ...
    virtual_start_and_end = -van_cost
    c[0][0]     = virtual_start_and_end
    c[0][I+1]   = virtual_start_and_end
    c[I+1][0]   = virtual_start_and_end
    c[I+1][I+1] = virtual_start_and_end
    ...
    return model.objVal, r_a, r_b, r_p, r_q, r_x, r_w_H, r_w_L, r_u, new_r_y, only_station_ID

# detail = pd.DataFrame(["ID","r_a", "r_b", "r_p", "r_q", "r_x", "r_w_H", "r_w_L", "r_u"])
detail = {}
def objective_value(K, A, S_H, S_L, L, lambda_value):
    objective_value = []
    # detail = pd.DataFrame(["r_a", "r_b", "r_p", "r_q", "r_x", "r_w_H", "r_w_L", "r_u"])
    
    for i in range(K):
        print("=========" + str(i) + "===============")
        objVal, r_a, r_b, r_p, r_q, r_x, r_w_H, r_w_L, r_u, r_y, name_list = each_run(i,A,S_H,S_L,L,lambda_value)
        detail[i] = {}
        detail[i]["ID"] = i
        ...
        # detail.iloc[i]=[objVal, r_a, r_b, r_p, r_q, r_x, r_w_H, r_w_L, r_u]
        objective_value.append((i,objVal))
    # print(objective_value)
    return objective_value


# K, A,S_H,S_L,L,lambda_value



# objective_value(K, A, S_H, S_L, L, lambda_value)
...
print(objective_value(K,A,S_H,S_L,L,lambda_value))
