...
if __name__ == '__main__':
        ...
        for i in range(c):
            ...
            for j in range(1,len(a_list)-1):
                ...
                if solver_result[i]["r_a"][j] > 0:
                    ...
                    station_string[i][j] = "%s (取 %d)"%(Label[id], int(solver_result[i]["r_a"][j]+0.5))
                    ...
                    station_string[i][j] = "%s (放 %d)"%(Label[id], int(solver_result[i]["r_b"][j]+0.5))
                # print(i,j)
                # print(y_list)
    ...
    plots = sns.barplot([i+1 for i in range(cluster_k)], obj_sum)
    ...
    plt.ylim(0, obj_sum[0] *1.5)
    
    
    # Finally showing the plot
    plt.savefig('5_Objective values under different k.png')
    plt.show()
